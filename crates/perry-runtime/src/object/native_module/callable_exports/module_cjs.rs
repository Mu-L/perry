...
fn module_cjs_extension_function(name: &str) -> f64 {
    ...
    set_bound_native_closure_name(closure.get_raw_mut_ptr(), name);
...
fn module_wrap_value() -> f64 {
    ...
    let scope = crate::gc::RuntimeHandleScope::new();
    let closure = scope.root_raw_mut_ptr(closure);
    set_bound_native_closure_name(closure.get_raw_mut_ptr(), "wrap");
...
fn module_prototype_method(name: &str, length: u32) -> f64 {
    ...
    let scope = crate::gc::RuntimeHandleScope::new();
    let closure = scope.root_raw_mut_ptr(closure);
    set_bound_native_closure_name(closure.get_raw_mut_ptr(), "");
    set_builtin_closure_length(
        closure.get_raw_mut_ptr::<crate::closure::ClosureHeader>() as usize,
        length,
    );
