...
pub(crate) fn lower_native_method_call(
    ...
    let Some(recv) = object else {
        ...
        // Narrow to the value-form (receiver-less, no class scope): a call
        // with `class_name: Some(..)` keeps its prior class-scoped fallback.
        if class_name.is_none() && crate::nm_install::nm_install_symbol(module).is_some() {
            let recv_box =
